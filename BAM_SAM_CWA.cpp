...
static void show_matrix(float* matrix, int rows, int cols);
static void mm_strassen(float* matA, float* matB, float* matC, const int M, const int N, const int K);
static void mm_coppersmith_winograd(float* matA, float* matB, float* matC, const int M, const int N, const int K);
...
// Show Matrix (for debugging purposes & accuracy checking)
...
/*
 * Used in Coppersmith-Winograd Algorithm after cutoff
...
static void mm_CoppersmithWinograd(float* matA, float* matB, float* matC, const int M, const int N, const int K,
                             const int strideA, const int strideB, const int strideC){
    if((M <= 2) || (M%2 != 0 || N%2 != 0 || K%2 != 0)){
        return basic_matrix_multiplication(matA, matB, matC, M, N, K, strideA, strideB, strideC);
    }
...
static void mm_CoppersmithWinograd(float* matA, float* matB, float* matC, const int M, const int N, const int K){
	// Call Overloaded func with stride col nums
...
static void mm_test(int M, int N, int K, int rangeTop){
    ...
    clock_t start,end;
    for(int i = 0; i < 5; i++){
        float * mA = (float*) malloc(M*K*sizeof(float)); // matA
        float * mB = (float*) malloc(K*N*sizeof(float)); // matB
        float * mC = (float*) malloc(M*N*sizeof(float)); // matC
        float * mD = (float*) malloc(M*N*sizeof(float)); // matD
        float * mE = (float*) malloc(M*N*sizeof(float)); // matE
        
		// Fill matrices A and B with random values [0-100]
		for(int j = 0; j < M*K; j++){
            mA[j] = rand() % rangeTop;
        }
        for(int j = 0; j < K*N; j++){
        ...
        endtime = (double) (end-start)/CLOCKS_PER_SEC;
        printf("CWA%d time: %fms\n", i, endtime*1000);

		/*
		* Uncomment this section out if you wish to see the output of all calculated matrices and orginal
		* matrices A and B. Only useful for visually checking accuracy on smaller matrices.
		* 
        for(int j = 0; j < M*N; j++){
